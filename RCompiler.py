...
#=================================== Less than =================================
    if p[2] == "<":
        if p[1] in variables and p[3] in variables:
            p[0] = ('LT',variables[p[1]] < variables[p[3]])
        elif p[1] in variables:
            p[0] = ('LT',variables[p[1]] < p[3])
        elif p[3] in variables:
            p[0] = ('LT',p[1] < variables[p[3]])
        else:
            p[0] = ('LT',p[1] < p[3])
#==================================Grater than (GT)=============================
    elif p[2] == ">":
        if p[1] in variables and p[3] in variables:
            p[0] = ('GT',variables[p[1]] > variables[p[3]])
        elif p[1] in variables:
            p[0] = ('GT',variables[p[1]] > p[3])
        elif p[3] in variables:
            p[0] = ('GT',p[1] > variables[p[3]])
        else:
            p[0] = ('GT',p[1] > p[3])
#================================== Less Equal (LE)=============================
    elif p[2] == "<=":
        if p[1] in variables and p[3] in variables:
            p[0] = ('LE',variables[p[1]] <= variables[p[3]])
        elif p[1] in variables:
            p[0] = ('LE',variables[p[1]] <= p[3])
        elif p[3] in variables:
            p[0] = ('LE',p[1] <= variables[p[3]])
        else:
            p[0] = ('LE',p[1] <= p[3])
#================================== Greater Equal (GE)==========================
    elif p[2] == ">=":
        if p[1] in variables and p[3] in variables:
            p[0] = ('GE',variables[p[1]] >= variables[p[3]])
        elif p[1] in variables:
            p[0] = ('GE',variables[p[1]] >= p[3])
        elif p[3] in variables:
            p[0] = ('GE',p[1] >= variables[p[3]])
        else:
            p[0] = ('GE',p[1] >= p[3])
#================================== Equal (EQ)==================================
    elif p[2] == "==":
        if p[1] in variables and p[3] in variables:
            p[0] = ('EQ',variables[p[1]] == variables[p[3]])
        elif p[1] in variables:
            p[0] = ('EQ',variables[p[1]] == p[3])
        elif p[3] in variables:
            p[0] = ('EQ',p[1] == variables[p[3]])
        else:
            p[0] = ('EQ',p[1] == p[3])
#================================== Not Equal (NE)==============================
    elif p[2] == "!=":
        if p[1] in variables and p[3] in variables:
            p[0] = ('NE',variables[p[1]] != variables[p[3]])
        elif p[1] in variables:
            p[0] = ('NE',variables[p[1]] != p[3])
        elif p[3] in variables:
            p[0] = ('NE',p[1] != variables[p[3]])
        else:
            p[0] = ('NE',p[1] != p[3])
...
def p_simple_block(p):
    '''
        ...
        simple_block : IF conditional OPEN_BRACKET simplestmt CLOSE_BRACKET  ELSE_IF conditional OPEN_BRACKET simplestmt CLOSE_BRACKET ELSE OPEN_BRACKET simplestmt CLOSE_BRACKET
                     | IF conditional OPEN_BRACKET simplestmt CLOSE_BRACKET  ELSE OPEN_BRACKET simplestmt CLOSE_BRACKET
